...
import org.anyboot.mvc.plugin.springmvc.TemplateModelAndView;
import org.anyboot.mvc.plugin.springmvc.TemplateView;
import org.anyboot.util.BasicUtil;
import org.anyboot.util.BeanUtil;
import org.anyboot.util.ConfigTable;
import org.anyboot.util.DESUtil;
...
public class TemplateController extends AnybootController {
	...
	protected TemplateModelAndView template(boolean adapt, String name, String template){
		...
		tv.setViewName(name);
		tv.addObject(TemplateView.ANYBOOT_TEMPLATE_NAME, content_template);
		...
		try{
			tv.addObject(TemplateView.ANYBOOT_STYLE_TEMPLATE_DES, DESUtil.getInstance().encrypt(style_template));
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	protected TemplateModelAndView template(boolean adapt, String name){
		return template(adapt, name, TemplateView.ANYBOOT_TEMPLATE_NAME_DEFAULT);
	}

	protected TemplateModelAndView template(String name){
		return template(false, name, TemplateView.ANYBOOT_TEMPLATE_NAME_DEFAULT);
	}
