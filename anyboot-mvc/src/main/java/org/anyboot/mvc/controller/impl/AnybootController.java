...
import org.anyboot.controller.AbstractBasicController;
...
import org.anyboot.util.Constant;
import org.anyboot.util.FileUtil;
import org.anyboot.util.JSONDateFormatProcessor;
...
public class AnybootController extends AbstractBasicController {

	@Autowired(required = false)
	@Qualifier("anybootService")
	protected AnybootService service;
	...
	public String result(boolean result, Object data, String message) {
		...
		map.put("code", "200");
    	map.put("request_time", getRequest().getParameter("_anyboot_request_time"));
    	map.put("response_time_fr", getRequest().getAttribute("_anyboot_response_time_fr"));
	 ...
	 * AJAX分页时调用 
	 * 分数数据在服务器生成
	 * @param data	数据 request.setAttribute("_anyboot_navi_data", data);
	...
	public String navi(HttpServletRequest request, HttpServletResponse response, DataSet data, String page, Object ext){
		...
		if(null == data){
			data = (DataSet)request.getAttribute("_anyboot_navi_data");
		}else{
			request.setAttribute("_anyboot_navi_data", data);
		}
		PageNavi navi = null;
		if(null != data){
			navi = data.getNavi();
