...
import org.anyboot.util.NumberUtil;
import org.anyboot.web.tag.Navi;
...
//	public static final String PAGE_NO				= "pageNo"								;
	...
	private String style		= ""	; //样式标记对应anyboot-navi.xml中的config.key
	...
	public String html(String creater){
		...
		if("ajax".equals(creater)){
			configVarKey = Navi.CONFIG_FLAG_KEY + flag;	//_anyboot_navi_conf_0
		}
		...
		//当前页
		builder.append("<input type='hidden' id='hid_cur_page_"+flag+"' name='"+config.KEY_PAGE_NO+"' class='_anyboot_navi_cur_page' value='"+curPage+"'/>\n");
		//共多少页
		builder.append("<input type='hidden' id='hid_total_page_"+flag+"' name='"+config.KEY_TOTAL_PAGE+"' class='_anyboot_navi_total_page' value='"+totalPage+"'/>\n");
		//共多少条
		builder.append("<input type='hidden' id='hid_total_row_"+flag+"' name='"+config.KEY_TOTAL_ROW+"' class='_anyboot_navi_total_row' value='"+totalRow+"'/>\n");
		//每页显示多少条
		if(config.VAR_CLIENT_SET_VOL_ENABLE){
			builder.append("<input type='hidden' id='hid_page_rows_key_"+flag+"'  class='_anyboot_navi_page_rows_key' value='"+config.KEY_PAGE_ROWS+"'/>\n");
			builder.append("<input type='hidden' id='hid_page_rows_"+flag+"' name='"+config.KEY_PAGE_ROWS+"' class='_anyboot_navi_page_rows' value='"+pageRows+"'/>\n");
		}
		if("ajax".equals(creater)){
			builder.append("<input type='hidden' class='"+Navi.CONFIG_FLAG_KEY+"' value='" + flag + "'/>");
		}
		builder.append(createHidParams(config));
		builder.append("<div class=\"anyboot_navi\">\n");
		//数据统计
		...
		if(type ==0){ //下标导航
			...
			if(showJump){
				...
				.append(curPage)
				.append("' class='navi-go-txt _anyboot_jump_txt' id='hid_jump_txt_"+flag+"' onkeydown='_navi_jump_enter("+configVarKey+")'/>")
