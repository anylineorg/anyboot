...
import org.anyboot.config.db.SQLCreater;
import org.anyboot.config.db.ds.DataSourceHolder;
import org.anyboot.config.db.run.RunSQL;
import org.anyboot.config.db.run.impl.TableRunSQLImpl;
import org.anyboot.config.db.run.impl.TextRunSQLImpl;
import org.anyboot.config.db.run.impl.XMLRunSQLImpl;
import org.anyboot.config.db.sql.auto.TableSQL;
import org.anyboot.config.db.sql.auto.TextSQL;
import org.anyboot.config.db.sql.xml.XMLSQL;
import org.anyboot.config.http.ConfigStore;
import org.anyboot.dao.PrimaryCreater;
import org.anyboot.entity.AnybootEntity;
import org.anyboot.entity.DataRow;
import org.anyboot.entity.DataSet;
import org.anyboot.exception.SQLException;
import org.anyboot.exception.SQLUpdateException;
import org.anyboot.util.BasicUtil;
import org.anyboot.util.BeanUtil;
import org.anyboot.util.ConfigTable;
import org.anyboot.util.SQLCreaterUtil;
...
public abstract class BasicSQLCreaterImpl implements SQLCreater{
	...
	public RunSQL createDeleteRunSQL(String dest, Object obj, String ... columns){
		...
		}else if(obj instanceof AnybootEntity){
			run = createDeleteRunSQLFromEntity(dest, (AnybootEntity)obj, columns);
		}
		return run;
	}
	...
	private RunSQL createDeleteRunSQLFromEntity(String dest, AnybootEntity obj, String ... columns){
		TableRunSQLImpl run = new TableRunSQLImpl();
	...
	public RunSQL createInsertTxt(String dest, Object obj, boolean checkPrimary, String ... columns){
		...
		if(obj instanceof AnybootEntity){
			return createInsertTxtFromEntity(dest,(AnybootEntity)obj,checkPrimary, columns);	
		}
...
//		}
	...
	private RunSQL createInsertTxtFromEntity(String dest, AnybootEntity entity, boolean checkPrimary, String ... columns){
		RunSQL run = new TableRunSQLImpl();
		StringBuilder sql = new StringBuilder();
		List<Object> values = new ArrayList<Object>();
	...
	public RunSQL createUpdateTxt(String dest, Object obj, boolean checkPrimary, String ... columns){
		...
		if(obj instanceof AnybootEntity){
			return createUpdateTxtFromEntity(dest,(AnybootEntity)obj,checkPrimary, columns);
		}
		return null;
	}

	private RunSQL createUpdateTxtFromEntity(String dest, AnybootEntity entity, boolean checkPrimary, String ... columns){
		RunSQL run = new TableRunSQLImpl();
//		if(null == entity){
...
//		entity.processBeforeDisplay();	//显示之前预处理
	...
	public List<String> confirmInsertColumns(String dst, Object data, String ... columns){
		...
		}else if(data instanceof AnybootEntity){
			return confirmInsertColumns(dst, (AnybootEntity)data, columns);
		}
		return null;
	}
	/**
	 ...
	 * @return
	 */
	private List<String> confirmInsertColumns(String dst, AnybootEntity entity, String ... propertys){
		List<String> keys = null;/*确定需要插入的列*/
		if(null == entity){
			return new ArrayList<String>();
		}
	 ...
	 * @return
	 */
	private List<String> confirmUpdateColumns(String dst, AnybootEntity entity, String ... propertys){
		List<String> keys = null;/*确定需要更新的列*/
		if(null == entity){
			return new ArrayList<String>();
		}
