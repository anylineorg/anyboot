...
public class ConfigTable {
	...
	private synchronized static void loadConfig() {
		try {
			...
			for(File f:files){
				...
				if("anyboot-config.xml".equals(name)){
					loadConfig(f);
				}
			}
			...
			for(File f:files){
				String name = f.getName();
				if(name.startsWith("anyboot-config") && !"anyboot-config.xml".equals(name)){
					loadConfig(f);
				}
			}
		} catch (Exception e) {
	...
	private static void debug(){
		...
		try{
			...
			line("","*", true);
			line("anyboot Core " + version, " ", true);
			line("anyboot.org ", " ", true);
			line(""," ", true);
			line("MinVersion " + minVersion + "[" + time+"]", " ", true);
			line(""," ", true);
			line("","*", true);
			line(" github.con  git地址：https://github.com/anybootorg/anyboot.git", "", false);
			line(" github.com  git帐号：public@anyboot.org(anyboot111111)", "", false);
			line(" ", " ", false);
			line(" oschina.net git地址：https://git.oschina.net/anyboot/anyboot.git", "", false);
			line(" oschina.net svn地址：svn://git.oschina.net/anyboot/anyboot", "", false);
			line(" oschina.net 帐号密码：public@anyboot.org(111111)", "", false);
			line("","*", true);
			line(" Debug 状态设置:anyboot-config.xml:<property key=\"DEBUG\">false</property>", "", false);
